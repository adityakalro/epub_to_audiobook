...
- [Pocket-TTS](https://kyutai.org/next/tts) (lightweight CPU-based TTS, see demo on their website)
- [Kokoro TTS](https://huggingface.co/spaces/hexgrad/Kokoro-TTS) (usage of this is done through a local OpenAI endpoint)
- [MLX-Audio](https://github.com/Blaizzy/mlx-audio) (Qwen3-TTS, Kokoro, and other TTS models on Apple Silicon via MLX)
...
- For using *Pocket-TTS*, no API Key is required (runs locally on CPU)
- For using *MLX-Audio* (default: Qwen3-TTS; Kokoro and others on Apple Silicon), no API Key is required; requires Apple Silicon Mac (M1/M2/M3/M4) and `pip install mlx-audio`
- For *LLM TTS optimization* (Ollama), a local [Ollama](https://ollama.ai) server is required (e.g. `ollama serve` and a model such as `ollama pull llama3.2`)
...
- **TTS Provider Selection**: Easy switching between Azure, OpenAI, Edge, Piper, Pocket-TTS, and MLX-Audio with provider-specific options
...
2. **Choose your TTS provider** from the tabs (OpenAI, Azure, Edge, Piper, Pocket-TTS, or MLX-Audio)
3. **Configure provider-specific settings**:
   ...
   - **Pocket-TTS**: Select from built-in voices or provide custom voice file (runs locally on CPU)
   - **MLX-Audio**: Model, voice, language code, speed (Apple Silicon only)
...
You can run each chapter's text through a local LLM (Ollama) before TTS to improve punctuation, emotion, and intonation for a more natural reading. Enable it with `--use_llm_optimization`. Long chapters are split at paragraph or sentence boundaries so the LLM context limit is not exceeded.

**Requirements:** A running [Ollama](https://ollama.ai) server (e.g. `ollama serve` and a model such as `ollama pull llama3.2`).

**Config file (optional):** Place `ollama_tts_config.json` in the current directory, or pass its path with `--ollama_config`. Keys: `ollama_url`, `ollama_model`, `ollama_max_chars` (max characters per chunk when splitting; default 4000). CLI flags override the config file: `--ollama_url`, `--ollama_model`, `--ollama_max_chars`.

**Example** `ollama_tts_config.json`:
...
```bash
python3 main.py book.epub output_folder --use_llm_optimization --tts=edge
python3 main.py book.epub output_folder --use_llm_optimization --ollama_model=llama3.2 --tts=azure
```
...
Pocket-TTS is a lightweight, CPU-based TTS engine that requires no GPU and no API keys. To use it:
...
- Faster than real-time generation (~6x on modern CPUs)
- Supports Python 3.10, 3.11, 3.12, 3.13, and 3.14
- Built-in voices: alba (female, British), marius, javert, jean, fantine, cosette, eponine, azelma
- Supports voice cloning from custom .wav files

**Usage:**
```bash
python3 main.py input.epub output_folder --tts=pocket --pocket_voice=alba
```

For more information, see the [Pocket-TTS GitHub repository](https://github.com/kyutai-labs/pocket-tts) and [model card](https://huggingface.co/kyutai/pocket-tts).

### MLX-Audio

MLX-Audio runs TTS on Apple Silicon (M1/M2/M3/M4) using the [MLX framework](https://github.com/Blaizzy/mlx-audio). The default model is **Qwen3-TTS** (0.6B Base). No API key is required.

```bash
pip install mlx-audio
python3 main.py input.epub output_folder --tts=mlx_audio --mlx_voice=Chelsie --mlx_lang_code=a
```

Options: `--mlx_model` (default: mlx-community/Qwen3-TTS-12Hz-0.6B-Base-bf16; use Kokoro-82M-bf16 for Kokoro), `--mlx_voice` (Qwen3: Chelsie, Drew, Emily, ...; Kokoro: af_heart, bf_alice, ...), `--mlx_lang_code` (a/b/j/z/e/f), `--mlx_speed` (Kokoro only). Requires Apple Silicon and ffmpeg for MP3/FLAC.
